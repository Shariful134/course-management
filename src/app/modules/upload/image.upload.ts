...
export const uploadToDigitalOceanAWS = async (
  ...
  try {
    ...
    const uniurl= `${file.fieldname}-${new Date().getTime()}`
    ...
    const Location = `${process.env.DO_SPACE_ENDPOINT}/${process.env.DO_SPACE_BUCKET}/${uniurl}`;

    console.log('digital ocean:>>>>',Location)
